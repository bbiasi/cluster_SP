...
# save(coord, df, agencia_mun, balanco, micro, file = "agrp_sp_ias.RData")

load("agrp_sp_ias.RData")
...
# Clustering ----
tree <- hclustgeo(dist0 , dist, alpha = range_alpha[6]) # alfa = 0.25
...
clusters_mun <- geobr::read_municipality(code_muni = "SP", year = 2007) %>%
  ...
  as.data.frame() %>% 
  dplyr::select(-geom) %>% 
  dplyr::mutate(cluster = ifelse(cluster == 1, "C1",
                                 ifelse(cluster == 2, "C2",
                                        ifelse(cluster == 3, "C3", "C4"))))
...
mapa1 <- muni_sp %>% 
  ...
  ggplot2::geom_sf(aes(fill = cluster),
                   size = .05, show.legend = FALSE, color = "white") +
  labs(subtitle = "Estado de São Paulo - Brasil", size = 8) +
  scale_fill_manual(values = c("#1270D8", "#40B5D2",	"#682BCD", "#9CE08F")) + 
  theme_minimal() +
...
# Modelo k = 5 por parciomonia
{
  set.seed(1)
  grid <- expand.grid(k = 5) # set k = 5
  index <- caret::createDataPartition(knn_mod_sNA$cluster, 
                                      p = 0.7, list = FALSE) 
  train <- knn_mod_sNA[index, ]
  test  <- knn_mod_sNA[-index, ]
  ...
  model_knnk5 <- caret::train(cluster ~ .,
                              data   = train,
                              method = "knn",
                              preProcess = c("scale", "center"),
                              trControl  = ctrl,
...
# Predict
final_model_knnk5 <- data.frame(Agrupado = test$cluster,
                                predict(model_knnk5, 
                                        newdata = test, type = "prob"))

final_model_knnk5 <- final_model_knnk5 %>% 
  tibble::rowid_to_column() %>% 
  # dplyr::rename(`C1` = "X1",
  #               `C2` = "X2",
  #               # `C4` = "X4" # removendo a cidade de SP
  #               `C3` = "X3") %>% 
  tidyr::gather(Predict_Cluster, Probabilidade, 3:5) %>% # 3:6 se existir coluna C4
  dplyr::group_by(rowid) %>% 
  dplyr::slice(which.max(Probabilidade))
final_model_knnk5x <- final_model_knnk5 %>% 
  # dplyr::mutate(Predict  = ifelse(Predict_Cluster == "C1", 1, 
  #                                 ifelse(Predict_Cluster == "C2", 2,
  #                                        ifelse(Predict_Cluster == "C3", 3, 4)))) %>% 
  # dplyr::mutate(Predict = as.factor(Predict),
  #               Agrupado = as.factor(Agrupado)) %>% 
  dplyr::mutate(Predict_Cluster = factor(Predict_Cluster, 
                                         levels = c("C1", "C2", "C3")))

cm_knn_k5 <- caret::confusionMatrix(final_model_knnk5x$Predict_Cluster,
                                    test$cluster)
cm_knn_k5
...
{
  set.seed(1)
  novo_dfs_na <- data.frame(predict(model_knnk5, 
